...
void MyTcpSocket::receiveMsg()
{
    ...
    case ENUM_MSG_TYPE_ADD_FRIEND_REJECT: //拒绝添加好友请求
    {
        [](PDU* pdu){
            char sourceName[32] = {'\0'};
            // 拷贝读取的信息
    ...
    case ENUM_MSG_TYPE_PRIVATE_CHAT_REQUEST: //私聊好友请求
    {
        resPdu=handlePrivateChatRequest(pdu);
        break;
    }
    case ENUM_MSG_TYPE_GROUP_CHAT_REQUEST:   //群聊好友请求
    {
        MyTcpSocket::handleGroupChatRequest(pdu);
        break;
    }
    case ENUM_MSG_TYPE_CREATE_DIR_REQUEST:  //创造文件夹请求
    {
    ...
    case ENUM_MSG_TYPE_DELETE_FILE_REQUEST: //删除文件夹请求
    {
        resPdu = handleDelFileOrDirRequest(pdu);
        break;
    }
    case ENUM_MSG_TYPE_RENAME_FILE_REQUEST: //重命名文件夹请求
    {
    ...
    case ENUM_MSG_TYPE_PRE_DIR_REQUEST://返回上一文件夹请求
    {
    ...
    if(NULL != resPdu)
    {
        qDebug() <<"uiMsgType:" <<resPdu -> uiMsgType << "caData:" << resPdu ->caData<<"caMsg:"<<resPdu->caMsg;
        this -> write((char*)resPdu, resPdu -> uiPDULen);
        // 释放空间
        free(resPdu);
        resPdu = NULL;
...
{
    DBOperate::getInstance().OfflineUser(m_strName.toStdString().c_str());
...
PDU* MyTcpSocket::handleRegistRequest(PDU* pdu)
{
    ...
    bool ret = DBOperate::getInstance().registerUser(caName, caPwd); // 处理请求，插入数据库
...
PDU* MyTcpSocket::handleLoginRequest(PDU* pdu)
{
    ...
    strncpy(caPwd, pdu -> caData + 32, 32);
    qDebug() << pdu -> uiMsgType << " " << caName << " " << caPwd;
    bool ret = DBOperate::getInstance().LoginUser(caName, caPwd); // 处理请求，插入数据库

    // 响应客户端
    PDU *resPdu = mkPDU(0); // 响应消息
    if(ret)
    {
        QString strUserRootPath = QString("%1/%2").arg(MyTcpServer::getInstance().getStrRootPath()).arg(caName); // 用户文件系统根目录
...
PDU *MyTcpSocket::handleOnlineUsersRequest()
{
...
PDU *MyTcpSocket::handleSearchUserRequest(PDU *pdu)
{
    char caName[32] = {'\0'};
    strncpy(caName, pdu -> caData, 32);
...
PDU *MyTcpSocket::handleAddFriendRequest(PDU *pdu)
{
    char addedName[32] = {'\0'};
    char sourceName[32] = {'\0'};
    // 拷贝读取的信息
...
PDU *MyTcpSocket::handleDeleteFriendRequest(PDU *pdu)
{
...
PDU *MyTcpSocket::handlePrivateChatRequest(PDU *pdu)
{
...
PDU *MyTcpSocket::handlePreDirRequest(PDU *pdu)
{
...
PDU *MyTcpSocket::handleUploadFileRequest(PDU *pdu, TransFile *transFile)
{
    char caCurPath[pdu -> uiMsgLen];
...
PDU *MyTcpSocket::handleDownloadFileRequest(PDU *pdu, QFile *fDownloadFile, QTimer *pTimer)
{
    char caFileName[32] = {'\0'};
    char caCurPath[pdu -> uiMsgLen];
...
PDU *MyTcpSocket::handleShareFileRequest(PDU *pdu, QString strSouName)
{
...
PDU *MyTcpSocket::handleShareFileNoteRespond(PDU *pdu)
{
    int iOldPathLen = 0;
    int iNewPathLen = 0;
